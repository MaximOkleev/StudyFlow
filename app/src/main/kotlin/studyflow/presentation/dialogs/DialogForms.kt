...
fun TaskDialog(subjects: List<Subject>, initial: StudyTask?, onDismiss: () -> Unit, onSave: (Long, String, String, TaskStatus, TaskPriority, Long?, Int?, Recurrence) -> Unit) {
    ...
    var customTaskText by remember { mutableStateOf("") }
    var customTemplateSelected by remember { mutableStateOf(false) }
        ...
        text = {
            Column(Modifier.heightIn(max = 590.dp).verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    FilterChip(selected = false, onClick = { customTemplateSelected = false; title = "День рождения"; description = "Тип: день рождения • Купить подарок / поздравить"; selectedSubjectId = null; recurrence = Recurrence.Yearly }, label = { Text("День рождения") })
                    FilterChip(selected = false, onClick = { customTemplateSelected = false; title = "Праздник"; description = "Тип: праздник • Ежегодное напоминание"; selectedSubjectId = null; recurrence = Recurrence.Yearly }, label = { Text("Праздник") })
                    FilterChip(selected = false, onClick = { customTemplateSelected = false; title = "Подготовиться к занятию"; description = "Повторить материалы и закрыть хвосты" }, label = { Text("Учеба") })
                    FilterChip(selected = customTemplateSelected, onClick = { customTemplateSelected = true; selectedSubjectId = null; title = customTaskText; description = "" }, label = { Text("Другое") })
                }
                OutlinedTextField(
                    customTaskText,
                    { value -> customTaskText = value; if (customTemplateSelected) title = value },
                    label = { Text("Другое") },
