...
function openTaskModal(id = "") {
  ...
  showModal(`
    ...
    <div class="form-grid">
      ...
      <label class="full">Другое<br><input id="taskOtherTitle" class="input wide" placeholder="Например: Позвонить, купить подарок, оформить документ" oninput="if(document.getElementById('taskTemplate').value==='other'){document.getElementById('taskTitle').value=this.value}" /></label>
...
function applyTaskTemplate(kind) {
  if (!kind) return;
  const otherInput = document.getElementById("taskOtherTitle");
  if (kind === "birthday") {
  ...
  if (kind === "other") {
    document.getElementById("taskSubject").value = "";
    document.getElementById("taskTitle").value = otherInput?.value || "";
