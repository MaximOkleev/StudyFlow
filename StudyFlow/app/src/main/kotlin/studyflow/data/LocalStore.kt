...
class LocalStore(rootDir: Path = Paths.get(System.getProperty("user.home"), ".studyflow"),
    ...
    fun save(subjects: List<Subject>, tasks: List<StudyTask>, notes: List<Note>, sessions: List<FocusSession>) {
        ...
        storeAtomically(file, p)
    }

    fun load(): StoreSnapshot? = loadFrom(file)
    ...
    fun importRawBackup(): Boolean {
        ...
        val snapshot = loadFrom(rawBackupFile) ?: return false
        save(snapshot.subjects, snapshot.tasks, snapshot.notes, snapshot.sessions)
        return true
    }

    private fun storeAtomically(target: Path, properties: Properties) {
        val tmp = target.resolveSibling("${target.fileName}.tmp")
        val bak = target.resolveSibling("${target.fileName}.bak")
        Files.newOutputStream(tmp).use { properties.store(it, "StudyFlow local data") }
        if (target.exists()) {
